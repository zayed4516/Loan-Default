...
st.markdown(
    ...
    "Predict the likelihood of loan default using advanced Machine Learning."
    "</p>",
    unsafe_allow_html=True
)
...
# ================================
# 4. Define mappings (must match training encodings)
# ================================
...
# ================================
# 5. Collect user inputs in two columns
# ================================
...
# ================================
# 6. Transform categorical inputs
# ================================
...
if st.button("🔍 Predict Loan Default", use_container_width=True):
    ...
    if prediction == 1:
        st.error(
            f"⚠️ This loan is likely to **default**.\n\n"
            f"**Default Probability:** {proba:.2%}"
        )
    else:
        st.success(
            f"✅ This loan is likely to be **paid back**.\n\n"
            f"**Repayment Probability:** {1-proba:.2%}"
        )
